...
try:
    dynamic_model = load_model('action.h5')
...
labels_dict_static = {i: label for i, label in enumerate(['J', 'K', 'Ñ', 'Q', 'X', 'Z'])}
labels_dict_dynamic = {0: 'J', 1: 'K', 2: 'Ñ', 3: 'Q', 4: 'X', 5: 'Z'}

# --- Variables para detección de movimiento y secuencias ---
previous_landmarks = None
movement_threshold = 0.0016
sequence = []
sequence_length = 30
last_spoken_char = ""
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        current_landmarks = np.array([[lmk.x, lmk.y, lmk.z] for lmk in hand_landmarks.landmark]).flatten()
        ...
        if previous_landmarks is not None:
            ...
            if movement < movement_threshold:
                ...
                for landmark in hand_landmarks.landmark:
                    ...
                    data_aux_static.append(landmark.y - wrist_y)
                
                prediction_static = static_model.predict([np.asarray(data_aux_static)])
                predicted_character = prediction_static[0]
                ...
                if confidence_counter >= CONFIDENCE_THRESHOLD:
                    if current_char_candidate != last_spoken_char:
                        ...
                        # GUARDAR EN ARCHIVO TEMPORAL PARA MODO APRENDIZAJE
                        try:
                            with open('last_detection.txt', 'w') as f:
                                f.write(current_char_candidate)
                        except:
                            pass
                ...
                # Seña dinámica
                data_aux_dynamic = current_landmarks
                sequence.append(data_aux_dynamic)
                ...
                if len(sequence) == sequence_length:
                    ...
                    prediction_dynamic = dynamic_model.predict(sequence_array, verbose=0)
